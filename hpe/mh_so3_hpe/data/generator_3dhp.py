    ...
    def __init__(
        ...
        if train == True:
            for key in poses_2d.keys():
                assert (
                    poses_3d is None or poses_2d[key].shape[0] == poses_3d[key].shape[0]
                )
            ...
            for key in poses_2d.keys():
                ...
                bounds_high = bounds[valid_frame[key].astype(bool)] + np.ones(
                    bounds_low.shape[0], dtype=int
                )
        ...
        if poses_3d is not None:
            self.batch_3d = np.empty(
                (
                    batch_size,
                    chunk_length,
                    poses_3d[key].shape[-2],
                    poses_3d[key].shape[-1],
                )
            )
