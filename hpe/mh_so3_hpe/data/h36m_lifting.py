...
# Joints in H3.6M -- data has 32 joints, but only 17 that move; these are the
# indices.
H36M_NAMES = [""] * 32
H36M_NAMES[0] = "Hip"
H36M_NAMES[1] = "RHip"
...
T_POSE_OPERATORS = {
    1: torch.tensor([1, 0, 0], dtype=torch.float),
    2: torch.tensor([0, -1, 0], dtype=torch.float),
    3: torch.tensor([0, -1, 0], dtype=torch.float),
    4: torch.tensor([-1, 0, 0], dtype=torch.float),
    5: torch.tensor([0, -1, 0], dtype=torch.float),
    6: torch.tensor([0, -1, 0], dtype=torch.float),
    7: torch.tensor([0, 1, 0], dtype=torch.float),
    8: torch.tensor([0, 1, 0], dtype=torch.float),
    9: torch.tensor([0, 1, 0], dtype=torch.float),
    10: torch.tensor([0, 1, 0], dtype=torch.float),
    11: torch.tensor([-1, 0, 0], dtype=torch.float),
    12: torch.tensor([-1, 0, 0], dtype=torch.float),
    13: torch.tensor([-1, 0, 0], dtype=torch.float),
    14: torch.tensor([1, 0, 0], dtype=torch.float),
    15: torch.tensor([1, 0, 0], dtype=torch.float),
    16: torch.tensor([1, 0, 0], dtype=torch.float),
}
...
h36m_skeleton = Skeleton(
    ...
    joints_names=H36M_NAMES,
    t_pose_operators=T_POSE_OPERATORS,
)

h36m_skeleton_joints_group_16 = [
    [2, 3],
    [5, 6],
    [1, 4],
    [0, 7],
...
h36m_cameras_intrinsic_params = [
    ...
    },
    {
        ...
        "tangential_distortion": [
            -0.0016190266469493508,
            -0.0027408944442868233,
        ],
        "res_w": 1000,
        "res_h": 1000,
        "azimuth": -70,  # Only used for visualization
    ...
    },
    {
        ...
        "tangential_distortion": [
            -0.0005872055771760643,
            -0.0018133620033040643,
        ],
        "res_w": 1000,
        "res_h": 1002,
        "azimuth": -110,  # Only used for visualization
...
class Human36mDataset(MocapDataset):
    def __init__(self, path, remove_static_joints=True, n_joints=16):
        ...
        if remove_static_joints:
            if self._n_joints == 16:
                ...
                # Set joints group
                self._skeleton._joints_group = h36m_skeleton_joints_group_16
                self._skeleton._compute_metadata()
            elif self._n_joints == 17:
                # Bring the skeleton to 17 joints instead of the original 32
                # (Copied from VideoPose3D code)
                self.remove_joints(
