...
class MixSTE(nn.Module):
    def __init__(
        ...
        self.Spatial_patch_to_embedding = nn.Linear(
            in_chans,
            embed_dim,
        )
        ...
        if mup:
            last_layer = MuReadout(embed_dim, out_dim)
        else:
            last_layer = nn.Linear(embed_dim, out_dim)
...
class Attention(nn.Module):
    def __init__(
        ...
        default_scale = 1 / head_dim if mup else head_dim**-0.5
        self.scale = qk_scale or default_scale
...
class MuAttention(Attention):
    def __init__(
        ...
        head_dim = dim // num_heads
        # muP attention scaling: 1/d instead of 1/sqrt(d)
        self.scale = qk_scale or 8 / head_dim
