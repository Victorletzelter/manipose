...
ave_mpjpe_per_beta = np.array(
    [53.3, 54.6, 83.6]
)  # first value uses mup=False ... --> fix
ora_mpjpe_per_beta = np.array(
    [47.4, 52.0, 82.8]
)  # first value uses mup=False ... --> fix
...
fig, ax = plt.subplots(1, 1, figsize=(TEXT_WIDTH, 0.35 * TEXT_WIDTH))
c_pal = sns.color_palette("muted")
ax.plot(
    n_hyps, ave_mpjpe, "--", marker="o", lw=2, label="Aggregated MPJPE", color=c_pal[1]
)
...
setup_style(grid=True, column_fig=True, fontsize=7)
fig, (ax_k, ax_beta) = plt.subplots(1, 2, figsize=(TEXT_WIDTH, 0.35 * TEXT_WIDTH))
c_pal = sns.color_palette("muted")
ax_k.plot(
    n_hyps, ave_mpjpe, "--", marker="o", lw=2, label="Aggregated MPJPE", color=c_pal[1]
)
...
ax_beta.plot(
    ...
    marker="o",
    lw=2,
    label="Aggregated MPJPE",
    color=c_pal[1],
)
ax_beta.plot(
    beta, ora_mpjpe_per_beta, marker="s", lw=2, label="Oracle MPJPE", color=c_pal[1]
)
ax_beta.set_xticks([0.1, 0.5, 1.0])
