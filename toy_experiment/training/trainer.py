...
class Trainer:
    def __init__(
        ...
        if isinstance(model, ConstrainedMlpRmcl) or isinstance(
            model, ConstrainedMlpRmclV2
        ):
            self.mcl_enabled = True
        if (
            isinstance(model, ConstrainedMlpRmcl)
            or isinstance(model, ConstrainedMlpRmclV2)
            or isinstance(model, ConstrainedMlpV2)
            or isinstance(model, ConstrainedMlp)
        ):
    ...
    def loss_func_constrained(self, loss_func, pred, y, joint_prediction=False):
        if joint_prediction == False:
            preds_joints1, preds_joints2 = self.toruspoints_to_joints(
                pred, major_radius=self.major_radius, minor_radius=self.minor_radius
            )  # (B,3),(B,3)
        else:
            preds_joints1 = pred[:, :3]  # (B,3)
            preds_joints2 = pred[:, 3:]  # (B,3)
        y_joint1, y_joint2 = self.toruspoints_to_joints(
            y, major_radius=self.major_radius, minor_radius=self.minor_radius
        )  # (B,3),(B,3)
        loss = (1 / 2) * (
            loss_func(preds_joints1, y_joint1) + loss_func(preds_joints2, y_joint2)
        )
        return loss
    ...
    def train(
        ...
        if self.enabled_3d is True:
            ...
            if self.joints_prediction is True and self.enabled_3d is True:
                loss_func = lambda pred, y: self.loss_func_constrained(
                    loss_func_past, pred, y, joint_prediction=True
                )
            elif self.joints_prediction is False and self.enabled_3d is True:
                loss_func = lambda pred, y: self.loss_func_constrained(
                    loss_func_past, pred, y, joint_prediction=False
                )
    ...
    def eval(
        self,
        eval_sets: Tuple[TensorDataset],
        metric: Callable,
    ) -> Tuple[List[float], List[Tensor]]:
        predictions = list()
        performances = list()
        hypothesis_list = list() if self.diff_enabled or self.mcl_enabled else None
        with torch.no_grad():
        ...
        with torch.no_grad():
            for eval_set in eval_sets:
                ...
                if distribution is not None:
                    angles = distribution.torus_cartesian_to_angles_batch(
                        major_radius=2, minor_radius=1, points=y_eval.cpu().numpy()
                    )
                ...
                Xy_eval = np.concatenate(
                    [X_eval.cpu().numpy(), y_eval.cpu().numpy()], axis=1
                )
    ...
    def toruspoints_to_joints(self, vector, major_radius=2, minor_radius=1):
        ...
        joint1 = torch.cat(
            (joint1, torch.zeros(size=(B, 1), device=joint1.device)), axis=1
        )
        joint2 = vector
