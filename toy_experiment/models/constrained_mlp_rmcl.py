...
class ConstrainedMlpRmcl(ConstrainedMlp):
    ...
    def wta_with_scoring_l2_loss(
        ...
        gt_scores[batch_indices, active_heads_idx] = 1.0
...
class ConstrainedMlpRmclV2(ConstrainedMlpV2):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        predictions = list()
        for head in self.fc_out:
            angles, scores_logits = torch.split(
                head(input), split_size_or_sections=[2, 1], dim=1
            )  # theta: (B,2), score_logits: (B,1)
            cartesian_positions_predicted = self.torusanglestocartesian(
    ...
    def wta_with_scoring_l2_loss(
        ...
        base_loss = joint_l2_loss_per_hyp(
...
def joint_l2_loss_per_hyp(
    ...
    joint1_hypothesis_pred, joint2_hypothesis_pred = torushyps_to_joints(
        hypothesis_pred, R=major_radius, r=minor_radius
    )  # (B, H, 3), (B, H, 3)
    joint1_y, joint2_y = toruspoints_to_joints(
        y, R=major_radius, r=minor_radius
    )  # (B, 3)

    err_joint_1 = torch.mean(
        (
            joint1_hypothesis_pred
            - joint1_y[:, None, :].expand_as(joint1_hypothesis_pred)
        )
        ** 2,
        dim=2,
    )  # (B,H)
    err_joint_2 = torch.mean(
        (
            joint2_hypothesis_pred
            - joint2_y[:, None, :].expand_as(joint2_hypothesis_pred)
        )
        ** 2,
        dim=2,
    )  # (B,H)
    assert err_joint_1.shape == err_joint_2.shape
    assert err_joint_1.shape == (
        joint1_hypothesis_pred.shape[0],
        joint1_hypothesis_pred.shape[1],
    )
